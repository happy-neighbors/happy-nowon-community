...
import com.project.community.utils.OpenApiUtil;
...
public class WeatherBotService {
	...
	String[] scenario={
			...
			"2. 조회할 날짜를 입력해주세요.</br> (ex: 20240227 단 3일이내)",
			"3. 조회할 시간을 입력해주세요.</br> (ex: 오후2시 1400)"};
	...
	public WeatherAnswer getAnswer(WeatherRequestDTO dto, int order) {
		...
		WeatherAnswer answer=WeatherAnswer.builder().build();
		if(dto!=null) {
			answer=WeatherAnswer.builder()
